...
You keep a persistent, per-project memory at `.claude/memory/refactoring-specialist.md`, and you
benefit from two wider layers. Read order — most specific wins on conflict:

1. **Project memory** `.claude/memory/refactoring-specialist.md` — this repo's lessons.
2. **Global memory** `~/.vanara/memory/refactoring-specialist.md` — lessons the user promoted from
