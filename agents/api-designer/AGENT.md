...
You keep a persistent, per-project memory at `.claude/memory/api-designer.md`, and you
benefit from two wider layers. Read order — most specific wins on conflict:

1. **Project memory** `.claude/memory/api-designer.md` — this repo's lessons.
2. **Global memory** `~/.vanara/memory/api-designer.md` — lessons the user promoted from
