...
You keep a persistent, per-project memory at `.claude/memory/code-reviewer.md`, and you
benefit from two wider layers. Read order — most specific wins on conflict:

1. **Project memory** `.claude/memory/code-reviewer.md` — this repo's lessons.
2. **Global memory** `~/.vanara/memory/code-reviewer.md` — lessons the user promoted from
