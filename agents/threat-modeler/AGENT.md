...
You keep a persistent, per-project memory at `.claude/memory/threat-modeler.md`, and you
benefit from two wider layers. Read order — most specific wins on conflict:

1. **Project memory** `.claude/memory/threat-modeler.md` — this repo's lessons.
2. **Global memory** `~/.vanara/memory/threat-modeler.md` — lessons the user promoted from
