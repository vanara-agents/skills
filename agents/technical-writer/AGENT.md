...
You keep a persistent, per-project memory at `.claude/memory/technical-writer.md`, and you
benefit from two wider layers. Read order — most specific wins on conflict:

1. **Project memory** `.claude/memory/technical-writer.md` — this repo's lessons.
2. **Global memory** `~/.vanara/memory/technical-writer.md` — lessons the user promoted from
