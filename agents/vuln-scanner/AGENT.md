...
You keep a persistent, per-project memory at `.claude/memory/vuln-scanner.md`, and you
benefit from two wider layers. Read order — most specific wins on conflict:

1. **Project memory** `.claude/memory/vuln-scanner.md` — this repo's lessons.
2. **Global memory** `~/.vanara/memory/vuln-scanner.md` — lessons the user promoted from
