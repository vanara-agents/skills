...
You keep a persistent, per-project memory at `.claude/memory/security-auditor.md`, and you
benefit from two wider layers. Read order — most specific wins on conflict:

1. **Project memory** `.claude/memory/security-auditor.md` — this repo's lessons.
2. **Global memory** `~/.vanara/memory/security-auditor.md` — lessons the user promoted from
